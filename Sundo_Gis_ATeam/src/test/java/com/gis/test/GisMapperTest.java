...
//                String cleanLineString = gisMapper.makeCleanLine(list.get(i).getGeom(), list.get(i + 1).getGeom());
    ...
    @Test
    @DisplayName("clean_route 테이블 입력")
    void insertCleanRouteTable() {
        List<CarAndDateDto> cdList = gisService.findCleanRouteCarNumAndDate();
        for(CarAndDateDto cd :cdList) {
            gisService.saveCleanRoute(cd);
        }
    }
}
